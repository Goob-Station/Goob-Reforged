...
public sealed class SupermatterComponent : Component
{
    ...
    /// <summary>Affects delamination timer. If removed - delamination timer is divided by 2.</summary>
    [DataField]
    ...
    /// <summary>Multiply outgoing rads by this. Generic knob.</summary>
    [DataField]
    ...
    /// <summary>we yell if over 50 damage every YellTimer Seconds</summary>
    [DataField]
    public float YellTimer { get; } = 60f;

    /// <summary>set to YellTimer at first so it doesnt yell a minute after being hit</summary>
    [ViewVariables(VVAccess.ReadOnly)]
    public float YellAccumulator { get; set; } = 60f;
    ...
    /// <summary>The message timer</summary>
    [DataField]
    public float SpeakAccumulator { get; set; } = 60f;

    [ViewVariables(VVAccess.ReadOnly)]
    public float UpdateAccumulator { get; set; }

    [DataField]
    public float UpdateTimer { get; } = 1f;

    [ViewVariables(VVAccess.ReadOnly)]
    public float ZapAccumulator { get; set; }

    [DataField]
    public float ZapTimer { get; } = 10f;
    #endregion SM Timer
    ...
    /// <summary>
    /// Damage per cycle can't increase by more % than this.
    /// It's inverse is effectively a minimum number of seconds for SM to delam.
    /// </summary>
    [ViewVariables(VVAccess.ReadOnly)]
    public float DamageHardcapPercentage { get; } = 0.002f;

    /// <summary>environmental damage is scaled by this</summary>
    [ViewVariables(VVAccess.ReadOnly)]
    ...
    /// <summary>The point at which we delamm</summary>
    [ViewVariables(VVAccess.ReadOnly)]
    ...
    public SoundSpecifier DelamAlarm { get; } = new SoundPathSpecifier("/Audio/Machines/alarm.ogg");
