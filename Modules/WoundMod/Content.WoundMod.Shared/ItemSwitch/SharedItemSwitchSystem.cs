...
using Content.WoundMod.Shared.Switchable;
using Content.WoundMod.Shared.Verbs;
using Robust.Shared.Audio.Systems;
using Robust.Shared.Network;

namespace Content.WoundMod.Shared.ItemSwitch;
    ...
    private void OnActivateVerb(Entity<ItemSwitchComponent> ent, ref GetVerbsEvent<ActivationVerb> args)
    {
        ...
        foreach (var state in ent.Comp.States.Where(state => !state.Value.Hidden)) // I'm linq-ing all over the place.
        {
            args.Verbs.Add(new ActivationVerb()
            {
                ...
                Category = WoundModVerbCategory.Switch,
        ...
        if (addedVerbs > 0)
            args.ExtraCategories.Add(WoundModVerbCategory.Switch);
    }
